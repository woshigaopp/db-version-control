 ...
 * dabao
 */
public class RollBackEntryPointcut extends StaticMethodMatcherPointcut implements ClassFilter {
    ...
    public RollBackEntryPointcut(String[] basePackages) {
        setClassFilter(this);
    ...
    private boolean matchesImpl(Method method, Class targetClass) {
        ...
        RollBackInvokeConfig ric = configMap.getByMethodInfo(key);
        if (ric == RollBackInvokeConfig.getNoRollBackInvokeConfigInstance()) {
            return false;
        } else if (ric != null) {
            return true;
        } else {
            ric = new RollBackInvokeConfig();

            RollBackConfigUtils.parse(ric, method);

            String name = method.getName();
            Class<?>[] paramTypes = method.getParameterTypes();
            parseByTargetClass(ric, targetClass, name, paramTypes);

            if (ric.getRollBackEntryAnnoConfig() == null) {
                configMap.putByMethodInfo(key, RollBackInvokeConfig.getNoRollBackInvokeConfigInstance());
                return false;
            } else {
                configMap.getTagMethodKeyMap().put(ric.getRollBackEntryAnnoConfig().getTag(), key);
                configMap.putByMethodInfo(key, ric);
                return true;
            }
        }
    }

    private void parseByTargetClass(RollBackInvokeConfig cac, Class<?> clazz, String name, Class<?>[] paramTypes) {
        ...
        if (matchThis) {
            ...
            for (Method method : methods) {
                if (methodMatch(name, method, paramTypes)) {
                    RollBackConfigUtils.parse(cac, method);
                    break;
                }
            }
        }
